...
from lib.models.nnFormer.utilities.nd_softmax import softmax_helper
from torch import nn
import torch
import numpy as np
from lib.models.nnFormer.initialization import InitWeights_He
from lib.models.nnFormer.neural_network import SegmentationNetwork
...
class nnFormer(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        if self.with_pmfs_block:
            skips[3] = self.global_pmfs_block(skips)
...
if __name__ == '__main__':
    ...
    model = nnFormer(crop_size=(64, 64, 64), input_channels=1, num_classes=35, with_pmfs_block=True).to(device)
