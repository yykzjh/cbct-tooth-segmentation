...
class DSC(object):
    ...
    @staticmethod
    def compute_per_channel_dsc(pred, label, num_classes):
        """
        计算各类别的DSC
        ...
        # 根据指标类型计算分母
        union_count = pred_count + label_count
        # 计算每个通道的dsc
        per_channel_dsc = (2 * intersect_count) / (union_count + 1e-12)
        # 将没有出现的类别的值置为-1.0
        for i in range(num_classes):
            if label_count[i] == 0:
                per_channel_dsc[i] = -1.0

        return per_channel_dsc
    ...
    def __call__(self, input, target):
        ...
        # 判断标注图和分割图的维度是否都是4维
        assert seg.dim() == target.dim() == 4, "DSC: 标注图和分割图的维度不是都为4维！"
        # 判断标注图和分割图的尺寸是否一致
        assert seg.size() == target.size(), "DSC: 分割图和标注图的尺寸不一致！"

        # 计算每个通道上的dsc
        per_channel_dsc = self.compute_per_channel_dsc(seg, target, self.num_classes)
        # 获取有效通道的mask
        mask = torch.ones((self.num_classes,))
        for i in range(self.num_classes):
            if per_channel_dsc[i] == -1.0:
                mask[i] = 0.0
        # 返回有效通道的平均iou
        return (torch.sum(per_channel_dsc * mask) / torch.sum(mask)).item()
